...
class mswblmService(win32serviceutil.ServiceFramework):
    ...
    def _eventHanlde(self):
        ...
        for log in logs:
            if LL is None:
                LL = []
            if log['sCommand'].find(r'刷卡计费') > -1 or log['sCommand'].find(r'客户端登录') > -1 or log['sCommand'].find(r'结帐') > -1:
                ...
                if info:
                    ...
                    if log['sCommand'].find(r'刷卡计费') > -1:
                        d['type'] = 1
                    elif log['sCommand'].find(r'客户端登录') > -1:
                        d['type'] = 2
                    elif log['sCommand'].find(r'结帐') > -1:
                        d['type'] = 3
                    else:
                        d['type'] = 3
                    LL.append(d)
    ...
    def _payEventHandle(self):
        ...
        for evt in payEvents:
            if evt['ComputerName'].find(r'会员充值') > -1:
...
if __name__ == '__main__':
    win32serviceutil.HandleCommandLine(mswblmService)